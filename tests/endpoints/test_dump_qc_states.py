...
def test_get_qc_by_product_id(test_client: TestClient, test_data_factory):
    ...
    BAD_CHECKSUM = "AAAAAAAAAAAAAAAAAAA"

    response = test_client.post("/pacbio/products/qc", json=[BAD_CHECKSUM])
    ...
    response = test_client.post(
        "/pacbio/products/qc", json=[BAD_CHECKSUM, FIRST_GOOD_CHECKSUM]
    )
    assert response.ok is True
    response_data = response.json()
    assert BAD_CHECKSUM not in response_data
    assert FIRST_GOOD_CHECKSUM in response_data
